...
def upgrade() -> None:
    ...
    run_status = postgresql.ENUM(
        "queued", "running", "completed", "failed", "cancelled", name="run_status", create_type=True
    )
    hitl_status = postgresql.ENUM(
        "pending", "approved", "rejected", "expired", name="hitl_status", create_type=True
    )
    incident_severity = postgresql.ENUM(
        "low", "medium", "high", "critical", name="incident_severity", create_type=True
    )
    incident_status_enum = postgresql.ENUM(
        "open", "investigating", "resolved", "closed", name="incident_status", create_type=True
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "status",
            sa.Enum("pending", "approved", "rejected", "expired", name="hitl_status"),
            nullable=False,
